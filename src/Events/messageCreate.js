    ...
    if (!message.member.permissions.has(command.userPermissions || [])) {
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(`** ❌ You don't Have ${command.userPermissions} To Run Command.. **`),
        ],
      });
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(`** ❌ I don't Have ${command.botPermissions} To Run Command.. **`),
        ],
      });
    } else if (cooldown(message, command)) {
      return message.channel.send(
        `*You are On Cooldown , wait \`${cooldown(message, command).toFixed()}\` Seconds*`,
      );
    } else {
