...
module.exports = async (client) => {
  try {
    ...
    fs.readdirSync("./src/SlashCommands").forEach((cmd) => {
      let commands = fs
        .readdirSync(`./src/SlashCommands/${cmd}/`)
  ...
  client.embed = (interaction, data) => {
    return interaction.followUp({
      embeds: [
        new MessageEmbed().setColor(ee.color).setDescription(data.substr(0, 2000)).setFooter({
          text: ee.footertext,
